<?php
/*
 * This file is part of the MonstrumSymfonyJCropBundle.
 *
 * (c) Erwin Eu <eu.erwin@gmail.com>
 */
namespace Monstrum\SymfonyJCropBundle\Command;
...
/**
 * Command to check and create tapmono-jcrop symlink into MonstrumSymfonyJCropBundle
 *
...
class SymfonyJCropSymlinkCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addArgument('pathTo' . static::$pathName, InputArgument::OPTIONAL, 'Where is tapmono/jcrop located?')
            ->addArgument('pathToMonstrumSymfonyJCropBundle', InputArgument::OPTIONAL, 'Where is SymfonyJCrop located?')
            ->addOption('force', 'f', InputOption::VALUE_NONE, 'Force rewrite of existing symlink if possible!')
            ->addOption('manual', 'm', InputOption::VALUE_NONE, 'If set please specify pathTo' . static::$pathName . ', and pathToMonstrumSymfonyJCrop')
            ...
            ->setName('monstrum:symfony-jcrop:symlink')
            ->setHelp(<<<EOT
The <info>monstrum:symfony-jcrop:symlink</info> command helps you checking and symlinking/mirroring the tapmono/jcrop library.

By default, the command uses composer to retrieve the paths of MonstrumSymfonyJCropBundle and tapmono/jcrop in your vendors.

If you want to control the paths yourself specify the paths manually:

php app/console monstrum:symfony-jcrop:symlink <comment>--manual</comment> <pathToSymfonyJCrop> <pathToMonstrumSymfonyJCrop>

Defaults if installed by composer would be :

pathToSymfonyJCrop:    ../../../../../../../../vendor/tapmono/jcrop/dist
pathToMonstrumSymfonyJCropBundle: vendor/monstrum/symfony-jcrop-bundle/Monstrum/SymfonyJCropBundle/Resources/public/symfony-jcrop
...
EOT
     ...
     */
    protected function getJCropName()
    {
        return self::$jcropName;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('manual')) {
			list($symlinkTarget, $symlinkName) = $this->getSymfonyJCropPathsFromUser();
            ...
            list($symlinkTarget, $symlinkName) = $cmanager->getSymlinkFromComposer(
                self::$monstrumSymfonyJCropBundleName,
                $this->getJCropName(),
                $options
            );
    ...
    protected function getSymfonyJCropPathsFromUser()
    {
        $symlinkTarget = $this->input->getArgument('pathTo' . static::$pathName);
        $symlinkName = $this->input->getArgument('pathToMonstrumSymfonyJCropBundle');

        if (empty($symlinkName)) {
            throw new \Exception("pathToMonstrumSymfonyJCropBundle not specified");
        }

        if (!is_dir(dirname($symlinkName))) {
            throw new \Exception("pathToMonstrumSymfonyJCropBundle: " . dirname($symlinkName) . " does not exist");
        }
